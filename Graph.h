...
class Graph 
{
    ...
    struct Edge
    {
        ...
        Edge(g_id id, node_ptr from, node_ptr to, EdgeData data) : id(id), from(from), to(to), data(data) {}
    };
    ...
    public:
        ...
        bool add_node(g_id id) { return add_node(id, NodeData()); } // Assumes that NodeData() exists
        ...
        bool add_edge(g_id from, g_id to) { return add_edge(from, to, EdgeData()); } // Assumes that EdgeData() exists
        ...
        bool remove_edge(g_id from, g_id to)
        {
            ...
            // TODO: Remove the proceeding code duplication
        ...
        bool node_exists(g_id node) const { return !(nodes.find(node) == nodes.end()); }

        bool edge_exists(g_id from, g_id to) const { return std::find_if(edges.begin(), edges.end(), [from, to](std::pair<g_id, edge_ptr> e) { return (e.second->from->id == from && e.second->to->id == to); }) != edges.end(); }
};